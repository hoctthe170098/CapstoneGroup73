...
namespace StudyFlow.Application.LichHocs.Queries.GetLichHocGiaoVien;
public record GetLichHocGiaoVienQuery : IRequest<Output>
{
    public int? Tuan {  get; init; }
    public int? Nam { get; init; }
}
public class GetLichHocGiaoVienQueryHandler : IRequestHandler<GetLichHocGiaoVienQuery, Output>
{
    ...
    public GetLichHocGiaoVienQueryHandler(IApplicationDbContext context
    ...
    public async Task<Output> Handle(GetLichHocGiaoVienQuery request, CancellationToken cancellationToken)
    {
        ...
        var giaoVienCode = await _context.GiaoViens
            .FirstOrDefaultAsync(gv => gv.UserId == UserId.ToString());
        if (giaoVienCode == null) throw new NotFoundIDException();
        var data = new GetLichHocGiaoVienByThuVaNamDto();
        ...
        }else if(request.Tuan != null && request.Nam != null)
        {
            ...
            for (int i = 2; i <= ngayTrongTuanList.Count+1; i++)
            {
                ...
                var lichHoc = await _context.LichHocs
                    .Where(lh => lh.GiaoVienCode == giaoVienCode.Code
